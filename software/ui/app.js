...
/* ═══════════════════════════════════════════════════════════════ */
document.getElementById('btn-cond-import').addEventListener('click', async () => {
  const res = await window.pywebview.api.browse_and_import_condition();
  if (!res || !res.ok) {
    if (res && res.error !== 'cancelled') _log('ERROR: ' + res.error);
    return;
  }
  _appendConditionRow(res.entry);
  _log(`Condition imported: ${res.entry.filename}`);
});

document.getElementById('btn-cond-delete').addEventListener('click', async () => {
  const fn = _selectedCondFilename();
  if (!fn) return;
  const res = await window.pywebview.api.delete_condition_file(fn);
  if (res && res.ok) {
    const tr = document.querySelector(`#cond-tbody tr[data-filename="${CSS.escape(fn)}"]`);
    if (tr) tr.remove();
    _log(`Condition deleted: ${fn}`);
  } else {
...
/*  Task management                                               */
/* ═══════════════════════════════════════════════════════════════ */
document.getElementById('btn-task-import').addEventListener('click', async () => {
  const res = await window.pywebview.api.browse_and_import_task();
  if (!res || !res.ok) {
    if (res && res.error !== 'cancelled') _log('ERROR: ' + res.error);
    return;
  }
  _appendTaskRow(res.filename, 'idle');
  _log(`Task imported: ${res.filename}`);
});
...
document.getElementById('btn-plot-log').addEventListener('click', async () => {
  ...
  function _replot() {
    const datasets = regNames
      ...
      .map(n => ({
        label: n,
        data: byReg[n].times.map((t, j) => ({ x: t, y: byReg[n].values[j] })),
